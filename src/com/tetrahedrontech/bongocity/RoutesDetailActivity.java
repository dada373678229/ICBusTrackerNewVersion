...
import com.tetrahedrontech.bongocity.API.coreAPI;
			...
			private class BusLocationMarkerThread extends AsyncTask<String, String, String> {
				...
				protected void onProgressUpdate(String... result){
					...
					for (int i=0; i<temp.length;i++){
						...
						busLocationMarkers.add(map.addMarker(new MarkerOptions().anchor((float)0.5, (float)0.5).flat(true).title("BUS").snippet(temp1[0]).position(busLocation).icon(BitmapDescriptorFactory.fromResource(R.drawable.bus_icon)).rotation(Integer.parseInt(temp1[3]))));
					}
				}
				/*
				@Override
